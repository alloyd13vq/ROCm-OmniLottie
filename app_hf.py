"""
OmniLottie Gradio Demo - Hugging Face Compatible Version

This version uses decoder_hf.py with from_pretrained() to load models,
supporting automatic downloading from Hugging Face Hub.
...
Usage:
    ...
    # Using local model
    MODEL_PATH=/path/to/local/model python app_hf.py

    # Custom configuration
    MODEL_PATH=your-org/your-model PROCESSOR_PATH=/path/to/processor python app_hf.py
"""
...
def load_model_once():
    ...
    # Load model using from_pretrained (supports HF Hub and local paths)
    model = LottieDecoder.from_pretrained(
        model_path,
        torch_dtype=runtime.torch_dtype,
        base_model_path=os.environ.get("PROCESSOR_PATH", "Qwen/Qwen2.5-VL-3B-Instruct"),
